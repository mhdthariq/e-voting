...
export default function OrganizationDashboard() {
  ...
  const [selectedElectionDetail, setSelectedElectionDetail] =
    useState<ElectionDetail | null>(null);
  const [loadingElectionId, setLoadingElectionId] = useState<number | null>(
    null,
  );
  ...
  const [selectedElectionForAssign, setSelectedElectionForAssign] = useState<{
    id: number;
    title: string;
  } | null>(null);
    ...
    try {
      ...
      if (res.ok) {
        ...
        localStorage.setItem(
          "user",
          JSON.stringify({ ...currentLocal, ...json.user }),
        );
      }
    } catch (e) {
  ...
  const loadElectionDetails = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/organization/elections/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (data.success) {
        setSelectedElectionDetail(data.data);
      }
    } catch (e) {
      console.error("Failed loading election detail:", e);
    } finally {
  ...
  const handleCreateSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setCreateSuccess(
          "Election created successfully! It is now in Draft mode.",
        );
  ...
  const filteredVoters = availableVoters.filter(
    (v) =>
  ...
  const endedElectionsList =
    stats?.detailedStatistics.filter((e) => e.electionStatus === "ENDED") || [];
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center py-4">
          <div>
            ...
            <p
              className={
                darkMode ? "text-gray-400 text-xs" : "text-gray-500 text-xs"
              }
            >
              ...
              >
                {currentUser?.profileImage ? (
                  ...
                  <span className="text-xs font-bold">
                    {getInitials(currentUser?.username || "ORG")}
                  </span>
                )}
              </div>
              <div className="hidden sm:block text-left overflow-hidden">
                <p
                  className={`text-sm font-medium leading-none truncate ${darkMode ? "text-emerald-100" : "text-gray-800"}`}
                >
                  {currentUser?.username}
                </p>
                <p
                  className={`text-[10px] mt-0.5 ${darkMode ? "text-emerald-500/70" : "text-gray-500"}`}
                >
                  Manage Org
                </p>
              </div>
      ...
      <main className="flex-grow max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-8 w-full">
              ...
              {[
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <div
                      className={`p-2 rounded-lg ${darkMode ? "bg-emerald-900/30 text-emerald-400" : "bg-emerald-50 text-emerald-600"}`}
                    >
                      <card.icon size={20} />
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              >
                ...
                <p className="text-sm opacity-70 mb-6">
                  Start a new voting session immediately.
                </p>
                <button
                  onClick={() => {
                    resetForm();
                  ...
                  className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-bold shadow-lg transition-all flex items-center justify-center gap-2"
                >
        ...
        {activeTab === "Elections" && stats && (
          ...
          >
            ...
            {stats.recentElections.length === 0 ? (
              <div
                className={`text-center py-12 rounded-xl border border-dashed ${darkMode ? "border-gray-700 text-gray-400" : "border-gray-300 text-gray-500"}`}
              >
                  ...
                  >
                    <div className="absolute top-4 right-4 z-10">
                      {updateLoading === e.id ? (
                        ...
                        <div className="relative group">
                          ...
                          <ChevronDown
                            size={12}
                            className={`absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none ${darkMode ? "text-white/50" : "text-black/50"}`}
                          />
                        </div>
                      )}
                    </div>
                    ...
                    <div className="mt-4 space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span
                          className={`font-bold ${e.participationRate > 50 ? "text-emerald-500" : "text-yellow-500"}`}
                        >
                          {e.participationRate}%
                        </span>
                      </div>
                    ...
                    <div
                      className={`mt-4 pt-3 border-t text-xs flex justify-between items-center ${darkMode ? "border-gray-800 text-gray-400" : "border-gray-100 text-gray-500"}`}
                    >
        ...
        {activeTab === "Voters" && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="overflow-hidden rounded-xl border border-gray-200 dark:border-emerald-900 shadow-sm"
          >
            {votersLoading ? (
              ...
              <div className="overflow-x-auto">
                <table
                  className={`min-w-full text-left text-sm ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                >
                  <thead
                    className={
                      darkMode
                  ...
                  >
                    {voters.map((v) => (
                      <tr
                        key={v.id}
                        className={`transition-colors ${darkMode ? "hover:bg-neutral-800" : "hover:bg-gray-50"}`}
                      >
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`px-2 py-1 rounded text-xs font-bold ${v.status === "active" ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"}`}
                          >
                            {v.status}
                          </span>
                        </td>
        ...
        {activeTab === "Results" && (
          ...
          >
            <div className="flex justify-between items-center">
              ...
              <p className="text-sm opacity-60">
                Analysis for {endedElectionsList.length} ended elections
              </p>
            </div>
            {endedElectionsList.length === 0 ? (
              <div
                className={`p-12 rounded-xl border border-dashed text-center ${darkMode ? "border-gray-700 text-gray-400" : "border-gray-300 text-gray-500"}`}
              >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Left: List */}
                <div
                  className={`lg:col-span-1 rounded-xl border overflow-hidden ${darkMode ? "bg-neutral-900 border-emerald-900" : "bg-white border-gray-200"}`}
                >
                  <div
                    className={`p-4 border-b font-semibold ${darkMode ? "border-emerald-900 bg-neutral-950/50" : "border-gray-100 bg-gray-50"}`}
                  >
                    Ended Elections
                  </div>
                  <div className="divide-y divide-gray-100 dark:divide-emerald-900/50 max-h-[600px] overflow-y-auto">
                    {endedElectionsList.map((election) => (
                      <div
                        ...
                        onClick={() => loadElectionDetails(election.electionId)}
                        className={`p-4 cursor-pointer transition-colors hover:bg-emerald-500/5 ${selectedElectionDetail?.id === election.electionId ? (darkMode ? "bg-emerald-900/20 border-l-4 border-l-emerald-500" : "bg-emerald-50 border-l-4 border-l-emerald-500") : "border-l-4 border-l-transparent"}`}
                      >
                        ...
                        <div className="flex justify-between mt-2 text-xs">
                          <span>
                            Turnout:{" "}
                            {/* Gunakan toFixed(1) agar hanya 1 angka di belakang koma (misal: 83.3%) */}
                            <span className="font-mono">
                              {Number(election.participationRate).toFixed(2)}%
                            </span>
                          </span>
                    ...
                    <>
                      ...
                      >
                        <h3 className="text-xl font-bold text-emerald-600 mb-2">
                          {selectedElectionDetail.title}
                        </h3>
                        <p className="text-sm opacity-60 mb-4">
                          {selectedElectionDetail.description}
                        </p>
                        <div className="grid grid-cols-3 gap-4">
                          <div className="text-center">
                            ...
                            <div className="text-2xl font-bold">
                              {selectedElectionDetail.stats.invited}
                            </div>
                          </div>
                          <div className="text-center">
                            <div className="text-xs opacity-60 mb-1">
                            ...
                            <div className="text-2xl font-bold text-emerald-500">
                              {selectedElectionDetail.stats.voted}
                            </div>
                          </div>
                          <div className="text-center">
                            <div className="text-xs opacity-60 mb-1">
                            ...
                            <div className="text-2xl font-bold text-blue-500">
                              {selectedElectionDetail.stats.participationRate.toFixed(
                                2,
                              )}
                              %
                            </div>
                      ...
                      {/* Two Column Layout: Participation Chart + Candidate Results */}
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        >
                          ...
                          <div className="flex-1 flex flex-col items-center justify-center relative">
                            {selectedElectionDetail.stats.invited === 0 ? (
                              ...
                              <>
                                <DonutChart
                                  ...
                                  data={[
                                    {
                                      label: "Voted",
                                      value: selectedElectionDetail.stats.voted,
                                      color: "#10B981",
                                    },
                                    {
                                      label: "Invited",
                        ...
                        >
                          ...
                          <div className="space-y-4">
                            {selectedElectionDetail.results.length === 0 ? (
                              <div className="text-center opacity-50 py-10">
                                No candidates found.
                              </div>
                            ) : (
                              selectedElectionDetail.results.map(
                                (candidate, idx) => {
                                  const totalVotes =
                                    selectedElectionDetail.stats.voted || 1;
                                  ...
                                  return (
                                    ...
                                    >
                                      <div className="flex justify-between items-end mb-1">
                                        <span
                                          className={`font-bold ${isWinner ? "text-amber-400 text-lg" : ""}`}
                                        >
                                          {isWinner && "👑 "}
                                          {candidate.name}
                                        </span>
                                        <span className="font-mono opacity-80">
                                          {candidate.voteCount} votes ({percent}
                                          %)
                                        </span>
                                      </div>
                                      <div
                                      ...
                                      {isWinner && (
                                        <div className="absolute -right-2 top-0 text-amber-500 text-[10px] font-bold rotate-12 opacity-0 group-hover:opacity-100 transition-opacity">
                                          WINNER
                                        </div>
                                      )}
                                    </div>
                          ...
                          {selectedElectionDetail.status === "ACTIVE" && (
        ...
        {isCreateModalOpen && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1 }}
              exit={{ scale: 0.95 }}
              className={`relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl shadow-2xl border ${darkMode ? "bg-neutral-900 border-emerald-800 text-white" : "bg-white border-gray-200 text-gray-900"}`}
            >
              <div
                className={`sticky top-0 z-10 flex justify-between items-center p-6 border-b ${darkMode ? "bg-neutral-900 border-emerald-800" : "bg-white border-gray-100"}`}
              >
                <h2 className="text-2xl font-bold text-emerald-600 flex items-center gap-2">
                  <CheckCircle className="w-6 h-6" /> Create New Election
                </h2>
                <button
              ...
              <div className="p-6 space-y-6">
                ...
                >
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full p-3 rounded-lg border focus:ring-2 focus:ring-emerald-500 outline-none ${darkMode ? "bg-neutral-800 border-emerald-700" : "bg-gray-50 border-gray-300"}`}
                      />
                    </div>
                    <div>
                  ...
                  <div
                    className={`p-4 rounded-xl border ${darkMode ? "bg-neutral-950/30 border-emerald-800" : "bg-gray-50 border-gray-200"}`}
                  >
                    ...
                    <div className="space-y-3 mb-4">
                      {candidates.map((c, idx) => (
                        <div
                          key={idx}
                          className={`flex justify-between items-center p-3 rounded-lg border ${darkMode ? "bg-neutral-800 border-emerald-700/50" : "bg-white border-gray-200"}`}
                        >
                          <div>
                      ...
                      {candidates.length === 0 && (
                        <p className="text-sm text-center opacity-50 italic py-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <input
                        ...
                        className={`p-2 rounded-lg border text-sm ${darkMode ? "bg-neutral-800 border-emerald-700" : "bg-white border-gray-300"}`}
                      />
                      <input
                        type="text"
                    ...
                    <button
                      ...
                      disabled={
                        !newCandidateName.trim() || !newCandidateDesc.trim()
                      }
              ...
              <div
                className={`p-6 border-t flex justify-end gap-3 ${darkMode ? "bg-neutral-900 border-emerald-800" : "bg-gray-50 border-gray-200"}`}
              >
                <button
                  type="button"
                  onClick={() => setIsCreateModalOpen(false)}
                  className={`px-5 py-2 rounded-lg font-medium text-sm transition-colors ${darkMode ? "hover:bg-neutral-800 text-gray-300" : "hover:bg-gray-200 text-gray-600"}`}
                >
                  Cancel
                </button>
                <button
        ...
        {isAssignModalOpen && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1 }}
              exit={{ scale: 0.95 }}
              className={`w-full max-w-lg rounded-xl shadow-2xl border flex flex-col max-h-[85vh] ${darkMode ? "bg-neutral-900 border-emerald-800 text-white" : "bg-white border-gray-200 text-gray-900"}`}
            >
              <div className="p-5 border-b border-gray-200 dark:border-emerald-800 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-xs opacity-60">
                    To: {selectedElectionForAssign?.title}
                  </p>
                </div>
              ...
              <div className="p-5 overflow-y-auto flex-1 space-y-4">
                <div
                  className={`flex items-center px-3 py-2 rounded-lg border ${darkMode ? "bg-neutral-800 border-emerald-700" : "bg-gray-50 border-gray-300"}`}
                >
                    ...
                    filteredVoters.map((voter) => {
                      ...
                      return (
                        <div
                          key={voter.id}
                          onClick={() =>
                            !isAssigned && toggleVoterSelection(voter.id)
                          }
                          className={`flex items-center justify-between p-3 rounded-lg border transition-all ${isAssigned ? (darkMode ? "bg-neutral-900 border-neutral-800 opacity-50 cursor-not-allowed" : "bg-gray-100 border-gray-200 opacity-50 cursor-not-allowed") : selectedVoterIds.includes(voter.id) ? "bg-emerald-500/10 border-emerald-500 cursor-pointer" : darkMode ? "bg-neutral-800 border-transparent hover:border-emerald-700 cursor-pointer" : "bg-gray-50 border-gray-200 hover:border-emerald-300 cursor-pointer"}`}
                        >
                          <div>
                          ...
                          <div
                            className={`w-5 h-5 rounded border flex items-center justify-center ${isAssigned ? "bg-gray-400 border-gray-400" : selectedVoterIds.includes(voter.id) ? "bg-emerald-500 border-emerald-500" : "border-gray-400"}`}
                          >
              ...
              <div className="p-5 border-t border-gray-200 dark:border-emerald-800 flex justify-end gap-3">
                ...
                >
                  {assignLoading
                    ? "Saving..."
                    : `Assign ${selectedVoterIds.length} Voters`}
                </button>
              </div>
            </motion.div>
          </motion.div>
