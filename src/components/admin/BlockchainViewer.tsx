...
import { ChevronDown, ChevronRight, Hash, Clock, Database, Link as LinkIcon, Lock } from "lucide-react";
...
export default function BlockchainViewer({ blocks, loading }: BlockchainViewerProps) {
    ...
    if (blocks.length === 0) {
        return (
            <div className="text-center py-12 opacity-50 border-2 border-dashed border-gray-700 rounded-xl">
                ...
                <p className="text-sm mt-2">New blocks are mined when votes are cast.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            <div className="relative">
                ...
                <div className="absolute left-6 top-6 bottom-6 w-0.5 bg-gradient-to-b from-emerald-500/20 to-emerald-500/5 -z-10 hidden md:block"></div>
                            ...
                            >
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    <div className="flex items-start gap-4">
                                        <div className={`mt-1 p-2 rounded-lg ${block.blockIndex === 0 ? "bg-emerald-500 text-black" : "bg-emerald-900/40 text-emerald-400"}`}>
                                            <Hash size={20} />
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                {block.blockIndex === 0 && (
                                                    <span className="px-2 py-0.5 rounded text-[10px] uppercase font-bold bg-emerald-500 text-black">Genesis</span>
                                                )}
                                            ...
                                            <p className="text-sm text-gray-400 flex items-center gap-2">
                                                ...
                                                Election: <span className="text-emerald-400">{block.election.title}</span>
                                            </p>
                                        </div>
                                    </div>

                                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 text-xs sm:text-sm text-gray-500 font-mono">
                                        <div className="flex items-center gap-1.5" title={new Date(block.timestamp).toLocaleString()}>
                                            <Clock size={14}/>
                                ...
                                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-2 text-[10px] md:text-xs font-mono bg-black/20 p-3 rounded-lg border border-white/5">
                                    <div className="truncate">
                                        <span className="text-gray-500 block mb-1">Previous Hash (Parent Link)</span>
                                        <span className="text-emerald-700/70">{block.previousHash}</span>
                                    </div>
                                    <div className="truncate">
                                        <span className="text-gray-500 block mb-1">Block Hash (Integrity Proof)</span>
                                        ...
                                        <div className="p-4 sm:p-6 space-y-4">
                                            ...
                                            <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                                                {block.parsedVotes.map((vote, i) => (
                                                    <div key={i} className="p-3 rounded bg-neutral-900 border border-gray-800 text-xs hover:border-emerald-900/50 transition-colors">
                                                        <div className="flex justify-between items-start mb-2">
                                                            <span className="font-bold text-emerald-500">Vote #{i+1}</span>
                                                            <span className="text-gray-600">{new Date(vote.timestamp).toLocaleString()}</span>
                                                        </div>
                                                        <div className="grid grid-cols-1 gap-1 text-gray-500 font-mono text-[10px]">
                                                            <div className="truncate">ID: {vote.voteId}</div>
                                                            <div className="truncate">Sig: {vote.signature.substring(0, 32)}...</div>
                                                        </div>
                                                    </div>
                                                ))}
                                                {block.parsedVotes.length === 0 && (
                                                    <p className="text-center py-4 text-xs opacity-50">No transactions in this block.</p>
                                                )}
                                            </div>
                                        </div>
                                    </motion.div>
